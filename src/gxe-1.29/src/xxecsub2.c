...
UTRACEP("utracelayout iter=%p,line=%p,runs=%p\n",iter,line,tmp_list);//~var8R~
  	...
  	{                                                              //~va20I~
    	...
    	{                                                          //~va20I~
			...
			;                                                      //~vaa7I~
#endif                                                             //~vaa7I~
			UTRACEP("%s:geometry ii=%d,offs_x=%d,offs_y=%d,width=%d\n",UTT,ii,pgeom->x_offset,pgeom->y_offset,pgeom->width);//~va20R~//~vbj2R~
    	}                                                          //~va20I~
...
UTRACEP("%s:geometry ii=%d/%d,offs_x=%d,offs_y=%d,width=%d cellw=%d\n",UTT,ii,pglyphstr->num_glyphs,pgeom->x_offset,pgeom->y_offset,pgeom->width,Pcellw);//~vbm8R~
...
UTRACED("usetmonospace_ligature Pdbcs",Ppdbcsxx,Plen);             //~vam0I~
...
//            		pgeom->y_offset=0;	//up when continued combining char//~va3uI~//~vbm0R~
            ...
            {	//COMBINE && not top                               //~vbm1I~
                ...
                UTRACEP("%s:combine not top glyphstrctr=%d,glyphctr=%d,geomwidth=%d\n",UTT,glyphstrctr,glyphctr,pgeom->width);//~vbmbR~//~vbmpR~
            }                                                      //~vbm1I~
          }//width=0                                               //~vbm1I~
...
//            else                                                 //~vamgR~
            ...
            UTRACEP("usetmonospace_ligature dbcsid=x%x,dbcscols=%d,wwglyph=%d,wwtotaldbcs=%d\n",*pdbcs,dbcsstep,wwglyph,wwtotaldbcs);//~vamgR~
...
//  		pgeomlast->width=diff;                                 //~va3uI~//~vb5hR~
        ...
        {                                                          //~va3uI~
            ...
            UTRACEP("usetmonospace_ligature adjusting sbcsonly=%d,wwtotaldbcs=%d,wwtotal=%d,Plen=%d,dbcscols=%d\n",swsbcsonly,wwtotaldbcs,wwtotal,Plen,dbcscols);//~vamgI~
        	wwtotal=0;                                             //~va3uI~
...
//      *Ppstrwidth=(wwtotal+PANGO_SCALE-1)/PANGO_SCALE;           //~vb4AR~//~vb5hR~
        *Ppstrwidth=(rc_wwtotal+PANGO_SCALE-1)/PANGO_SCALE;        //~vb5hI~
        UTRACEP("%s:return strwidth=%d,wwtotal=%d\n",UTT,*Ppstrwidth,rc_wwtotal);//~vb4AR~//~vb5hR~
    }                                                              //~vb4AI~
...
#endif   //!GTKPRINT                                               //~vam0I~
//===============================================================================//~va6yI~
//get default printer geometry                                     //~vamjR~
...
//#endif                                                           //~vamaR~
        ...
        UTRACEP("subgetcupspagesz form=%s,mark=%d,w=%lfmm,h=%lfmm,top=%d,left=%d,bot=%d,right=%d\n",//~vam0R~
...
//************************************                             //~vamuI~
    UTRACEP("cupsgetpapersizeGtk_fromPPD parm printer=%s,Mprinter=%s,form=%s\n",Pprinter,Mprinter,Ppform);//~vamuI~
    if (Pprinter)                                                  //~vamuI~
        pdest=Pprinter;                                            //~vamuI~
    else                                                           //~vamuI~
    if (strcmp(Mprinter,DEF_PRINTER) || !*Mprinter_cupsdefault)    //~vamuI~
    ...
    {                                                              //~vamuI~
        ...
        UTRACEP("cups margin inrect dest=%s,default=%s,ppd=%s\n",Mprinter,Mprinter_cupsdefault,pppdnm);//~vamuI~
        if (!pppdnm)                                               //~vamuI~
        {                                                          //~vamuI~
...
//    #endif                                                       //~vamuR~
    ...
    UTRACEP("csubgetpagesizeGtk_fromPPD iscustom=%d,w=%f point,h=%f point,t=%f,b=%f,l=%f,r=%f\n",gtk_paper_size_is_custom(pps),gtk_paper_size_get_width(pps,GTK_UNIT_POINTS),gtk_paper_size_get_height(pps,GTK_UNIT_POINTS),fmargins.top,fmargins.bottom,fmargins.left,fmargins.right);//~vamuR~
    return pps;                                                    //~vamuI~
...
//*******************************                                  //~vam0I~
    ...
    UTRACEP("csubsetpapersizeGtk parm form=%s,pps=%p,name=%s,displayname=%s\n",Pformtype,pps,gtk_paper_size_get_name(pps),gtk_paper_size_get_display_name(pps));//~vam0R~  //~vamsI~//~vamuR~
...
//*****************************                                    //~vbmfI~
    ...
    {                                                              //~vbmfI~
		rc=utf_iscombining(0,*Ppdddbcs,(int)ucs);                  //~vbmfR~
    }                                                              //~vbmfI~
    if (Ppchsz)                                                    //~vbmfI~
    	*Ppchsz=chsz;                                              //~vbmfI~
    if (Ppucs)                                                     //~vbmfI~
    	*Ppucs=ucs;                                                //~vbmfI~
    UTRACEP("%s:rc=%d,ucs=%06x,chsz=%d,dbcsid=%0x\n",UTT,rc,ucs,chsz,*Ppdddbcs);//~vbmfI~
    return rc;                                                     //~vbmfI~
