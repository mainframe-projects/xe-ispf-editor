...
//************************************                             //~v003I~
    ...
    Mscrwidth=ww;                                                  //~var8R~
    Mscrheight=hh;                                                 //~var8R~
    UTRACEP("xxemain_scrinit Gpview=%p w=%d,h=%d\n",Gpview,Mscrwidth,Mscrheight);//~var8R~
#endif                                                             //~var8R~
    UTRACEP("scrinit pixmap(view alloc) w=%d,h=%d\n",ww,hh);                          //~vX03R~//~vamrR~
#ifndef OPTGTK3                                                    //~var8R~
...
#endif                                                             //~var8R~
    dprintf("color %x=(%x,%x,%x)\n",Gcolordata.CDcolor.pixel,Gcolordata.CDcolor.red,Gcolordata.CDcolor.green,Gcolordata.CDcolor.blue);//~var8R~
...
//  gdk_gc_set_background(Gpgc,&(Gcolordata.CDcolor));             //~vXXER~
    ...
    UTRACEP("scrbgrect draw rect w=%d,h=%d\n",ww,hh);              //~var8R~
...
UTRACEP("maxcol=%d,maxrow=%d,ceilx=%d,ceily=%d,mcellw=%d,mcellh=%d\n",Mscrcmaxcol,Mscrcmaxrow,ceilx,ceily,Mcellw,Mcellh);//~var8R~
...
//        return 0;                                                //~vX01R~
    ...
    Mfulldrawsw=1;                                                 //~2914I~
    UTRACEP("wxescr onsize req invalidate\n");                     //~vam0I~
...
{                                                                  //~var8R~
//*******************************                                  //~var8R~
	dprintf("scrdrawline (%d,%d)->(%d,%d)\n",Pppoint->x,Pppoint->y,(Pppoint+1)->x,(Pppoint+1)->y);//~var8R~
...
//************************************                             //~2901I~
  ...
  UTRACEP("strput printsw=%d,preview=%d,row=%d,col=%d,len=%d,attr=%x,stype=%x\n",Pprintsw,Mpreviewmode,Prow,Pcol,Plen,Pattr,Pstyle);//~va1cR~//~va30R~//~vam0R~//~vam4R~
...
UTRACEP("scrdrawbrush draw rect xx=%d,yy=%d,ww=%d,hh=%d,bg=%x\n",xx,yy,ww,hh,Pbgpal);//~var8R~
#ifndef TEST                                                       //~vbm5R~
...
UTRACEP("drawtextstr printsw=%d,row=%d,xx=%d,yy=%d,len=%d\n",Pprintsw,Prow,Px,Py,Plen);//~va6pR~//~vam0R~
#ifdef GTKPRINT                                                    //~vamfI~
...
//  swligaturemode= (Mligature^Mligaturereverse)  //ligature mode  //~vb4rR~
    swligaturemode= WXE_LIGATUREMODE(Popt)  //ligature mode        //~vb4rI~
   					&&  !(optds & SPO_NOLIGATURE);	 //not ligature disabled part//~va6pI~
    UTRACEP("%s:swligaturemode=%x,Popt=%x,optd=%x\n",UTT,swligaturemode,Popt,optds);//~vbm5I~
  if (swutf8data)                                                  //~va20I~
  {                                                                //~va20I~
	opt=UTFDD2FO_ERRREP|UTFDD2FO_DBCSSPACEREP|UTFDD2FO_DUPREPCHK|UTFDD2FO_VIO;//~va20R~
    rc=xeutfcvdd2f(opt,Ptext,Ppdbcs,Plen,(UCHAR**)&ptext,&writelen,(UCHAR**)&pdbcs);//~va20R~
...
//  &&  !COMBINEMODE(Popt))  //split or unpr                       //~vb4AR~//~vbknR~
    ...
    {                                                              //~vb4AR~
      if (!swutf8data||!drawtextstr_ligatureu8_chkcombinesplit(optusms,Ppdc,pgc,xx,Py,pfg,pbg,ptext,writelen,pdata,pdddbcs,pdbcs,writelenlc))//draw combining char at top of str//~vbmcR~
      {                                                            //~vbmcI~
...
//  char wku8[8],wkdbcs[1]={0};                                    //~vb4AI~//~vbknR~
    ...
    GdkColor fgaltch={0,0/*r*/,0xffff/*g*/,0/*b*/};                //~vb4AR~
    char *pdddata;                                                 //~vbknI~
...
//	            usetmonospace_ligature(optusms,Gplayout,wku8,wkdbcs,1,Mcellw,0/*strwidth:xx*/);//~vb4AI~//~vbkpR~
            ...
            {                                                      //~vb4AI~
            	...
            	pango_layout_set_text(Gplayout,pu8,lenu8);         //~vb4AI~
...
#else                                                              //~vbkwI~
                usetmonospace_ligature(optusms|USMSO_SBCSCOMBSPLIT,Gplayout,pu8,plcdbcs,1,Mcellw,&strwidth); //split mode,chk 2 gliph and  2nd is width=0//~vbkwR~
#endif                                                             //~vbkwI~
            }                                                      //~vb4AI~
            UTRACEP("%s:draw_layout_with_colors xx=%d,altfg RGB r=%04x,g=%04x,b=%04x\n",UTT,xx,fgaltch.red,fgaltch.green,fgaltch.blue);//~vb4AI~
            gdk_draw_layout_with_colors(Ppdc,Ppgc,xx,Py,Gplayout,&fgaltch,Ppbg);//~vb4AI~
#ifndef OPTGTK3                                                    //~vb5iI~
#else   //OPTGTK3                                                  //~vb5iI~
...
{                                                                  //~vbknI~
    ...
    GdkColor fgaltch={0,0/*r*/,0xffff/*g*/,0/*b*/};                //~vbknI~
    char *pdddata;                                                 //~vbknI~
//************************************                             //~vbknI~
...
{                                                                  //~vbmaI~
    ...
    GdkColor fgaltch={0,0/*r*/,0xffff/*g*/,0/*b*/};                //~vbmaI~
    GdkColor *pfg;                                                 //~vbmaI~
//************************************                             //~vbmaI~
	UTRACEP("%s:opt=%04x,xx=%d,yy=%d,fg=%04x,bg=%04x\n",UTT,Popt,Px,Py,Ppfg,Ppbg);//~vbmaI~
...
{                                                                  //~vbkqI~
    ...
    GdkColor fgaltch={0,0/*r*/,0xffff/*g*/,0/*b*/};                //~vbmfI~
...
#else   //OPTGTK3                                                  //~vbkqM~
			csub_layout_set_textcolor(Gplayout,&fgaltch,Ppbg);//gdk_draw_layout_with_colors() ignore color parameter//~vbkqM~
#endif                                                             //~vbkqM~
...
{                                                                  //~vbkqI~
    ...
    GdkColor fgaltch={0,0/*r*/,0xffff/*g*/,0/*b*/};                //~vbmcI~
//*****************************                                    //~vbkqI~
...
//                optusms|=USMSO_SCM; //may contain Spacing Combining Mark(glpthctr>1)//~vbkqR~
          ...
          {                                                        //~vbmaI~
           ...
           if (Popt & USMSO_NOTCOMBINABLE)                         //~vbmcI~
    		gdk_draw_layout_with_colors(Ppdc,Ppgc,xx,Py,Gplayout,&fgaltch,Ppbg);//~vbmcI~
           else                                                    //~vbmcI~
...
//          gdk_draw_layout_with_colors(Ppdc,Ppgc,xx,Py,Gplayout,Ppfg,Ppbg);//~vbm2I~//~vbmfR~
            gdk_draw_layout_with_colors(Ppdc,Ppgc,xx,Py,Gplayout,&fgaltch,Ppbg);//~vbmfI~
#ifndef OPTGTK3                                                    //~vbm2I~
#else   //OPTGTK3                                                  //~vbm2I~
			csub_layout_set_textcolor(Gplayout,&fgaltch,Ppbg);//gdk_draw_layout_with_colors() ignore color parameter//~vbm2I~
#endif                                                             //~vbm2I~
